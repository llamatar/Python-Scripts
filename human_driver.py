...
def generate_human_driver_iterative_dict(
    max_drivers: int,
) -> dict[tuple[int, int, int], str]:
    """Returns a dict of (m, n, p) tuples to corresponding human driver strings."""
    return {
        (m, n, p): generate_human_driver_string(m, n, p)
        for n in irange(
            0, max_drivers
        )  # n has no theoretical upper bound, so it is capped by the given value
        for p in (0, 1)
...
def print_set(set_to_print: set) -> None:
    ...
    for element in sorted(set_to_print):
        print(
            f"{element}: {generate_human_driver_string(element[0], element[1], element[2])}"
        )
...
if __name__ == "__main__":
    ...
    try:
        user_input = int(
            input(
                "Please input the max number of drivers (anything greater than 20 will take a while to run): "
            )
        )
        ...
        if user_input < 0:
            ...
            print(
                "You've been warned... Press Ctrl+C to quit while the program is still running."
            )
    ...
    max_drivers_set = {
        element for element in element_set if element[1] <= max_drivers
    }  # filter out elements with more drivers than max_drivers
    ...
    if len(diff) == 0 or max_drivers == 0:
        print(
            "no differences between iterative version and EBNF version except the special cases (0, 0, 0) and (1, 0, 0)"
        )
    else:
